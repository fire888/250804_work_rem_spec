...
<details>
   <summary>Working on a task (developer)</summary>

   ---

   1. Для каждой новой задачи создать новую именную ветку от HEAD master. Вся работа делается в ней
   1. Commit messages есть аннотации, а не заголовки (краткое содержательное описание сделанного на eng)
   1. Когда задача готова, влить в ветку текущее состояние master (rebase или merge - не принципиально)
   1. Решить возникшие конфилкты слияния + push
   1. Создать запрос на слияние (с удалением ветки)
   1. Call to reviewer. PR
